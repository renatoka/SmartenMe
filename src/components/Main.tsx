...
const Main = () => {
  ...
  const handleWordReplacement = (event: any) => {
    event.preventDefault();
    if (inputSentence) {
      axios.post('https://smartenme.onrender.com/post', { sentence: inputSentence })
        .then((response) => { setOutputSentence(response.data); }
        )
      ...
      <div className="container mx-auto flex flex-col items-center justify-center h-full gap-3">
        ...
        <div className="flex flex-col w-full">
          {outputSentence ?
            ...
            :
            <form className="flex flex-col w-full" method="POST" name="smarten-me" data-netlify="true" data-netlify-honeypot="bot-field">
              ...
              <button className="w-full h-16 mt-4 rounded-md bg-[#ffdc15] text-[#2B2F3F] font-bold text-lg" onClick={handleWordReplacement}>Smart it</button>
            </form>
          }
        </div>
